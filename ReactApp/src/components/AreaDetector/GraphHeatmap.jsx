...
const PlotData = (props) => {
  ...
  const updateDataReducer = (pvs, newData) => {
    ...
    newPvs[newData.index] = {
      z: Z,
      type: "heatmap",
      showscale: props.showScale ?? false,
      marker: {
...
/**
 * The GraphHeatmap Component has been updated to Plotly.js scatter and line plot.
 * **Note**: The update includes a small breaking change.
 * See the backgroundColor prop for the workaround.
 */
const GraphHeatmap = (props) => {
  const theme = useTheme();
...
GraphHeatmap.propTypes = {
  ...
  /** Directive to show the heatmap scale*/
  showScale: PropTypes.number,
